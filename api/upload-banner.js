...
function readBody(req) {
  return new Promise((resolve, reject) => {
    let data = '';
    req.on('data', chunk => { data += chunk; if (data.length > 10e6) req.destroy(); });
...
module.exports = async (req, res) => {
  ...
  const chave = 'hero_banner_imagem';

  try {
    const sql = getDb();
    await sql`
      INSERT INTO configuracoes (chave, valor) VALUES (${chave}, ${dataUrl})
      ON CONFLICT (chave) DO UPDATE SET valor = ${dataUrl}, atualizado_em = NOW()
    `;
